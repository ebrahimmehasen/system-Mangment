...
export async function GET(
  ...
  const utf8 = encodeURIComponent(
    `${employee.name.replace(/[\\/:*?"<>|]+/g, "_").slice(0, 60)}-${stamp}.pdf`,
  );
  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="employee-report-${stamp}.pdf"; filename*=UTF-8''${utf8}`,
    },
  });
}
